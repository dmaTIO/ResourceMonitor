...
ipc.on("currentLoad", (event, data) => {
  ...
  document.getElementById("cpuPercentageValue").innerHTML =
    currentLoad.currentLoad.toFixed(2) + "%";
  document.getElementById("cpuPercentageBar").style.width =
    currentLoad.currentLoad.toFixed(2) + "%";
});

ipc.on("gpuLoad", (event, data) => {
  const gpuLoad = data;
  document.getElementById("gpuPercentageValue").innerHTML =
    gpuLoad.toFixed(2) + "%";
  document.getElementById("gpuPercentageBar").style.width =
    gpuLoad.toFixed(2) + "%";
});
...
ipc.on("disks", (event, data) => {
  ...
  diskBoxArray.forEach((el, idx) => {
    if (data[idx]) {
      ...
      el.getElementsByClassName("message-line")[0].innerHTML =
        (parseFloat(data[idx]["size"]) * (9.31 * Math.pow(10, -10))).toFixed(
          2
        ) + " GB";
      el.getElementsByClassName("message-line")[1].innerHTML =
        (
          parseFloat(data[idx]["available"]) *
          (9.31 * Math.pow(10, -10))
        ).toFixed(2) + " GB";
    } else {
      el.remove();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("pcName").innerHTML =
    osInfoStored?.hostname + " - " + osInfoStored?.distro;
  ...
  //Get GpuTotalMem
  const gpuTotalMemory = JSON.parse(
    window.localStorage.getItem("GPUmemoryTotal")
  );
  document.getElementById("gpuTotalMemory").innerHTML =
    "VRAM " + parseFloat(gpuTotalMemory) / 1000 + " GB";
  ...
  diskBoxArray.forEach((el, idx) => {
    if (fileSystem[idx]) {
      el.getElementsByClassName("name")[0].innerHTML =
        fileSystem?.[idx]["fs"] + "/";
      el.getElementsByClassName("message-line")[0].innerHTML =
        (
          parseFloat(fileSystem[idx]["size"]) *
          (9.31 * Math.pow(10, -10))
        ).toFixed(2) + " GB";
      el.getElementsByClassName("message-line")[1].innerHTML =
        (
          parseFloat(fileSystem[idx]["available"]) *
          (9.31 * Math.pow(10, -10))
        ).toFixed(2) + " GB";
    } else {
      el.remove();
